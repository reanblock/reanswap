...
contract ReanswapV2LibraryTest is Test {
    ...
    function testPairForNonexistentFactory() public {
        ...
        assertEq(pairAddress, 0x776AbB6f6Fd55ca99889Cd943ED473DD0B9aef62);
    }
    ...
    function testGetAmountsOut() public {
        ...
        uint256[] memory amounts = ReanswapV2Library.getAmountsOut(
            address(factory),
            0.1 ether,
            path
        );
    ...
    function testGetAmountsOutInvalidPath() public {
        ...
        ReanswapV2Library.getAmountsOut(address(factory), 0.1 ether, path);
    }

    function testGetAmountIn() public {
        uint256 amountIn = ReanswapV2Library.getAmountIn(
            1495,
            1 ether,
            1.5 ether
        );
    ...
    function testGetAmountInZeroInputAmount() public {
        vm.expectRevert(encodeError("InsufficientAmount()"));
        ReanswapV2Library.getAmountIn(0, 1 ether, 1.5 ether);
    }

    function testGetAmountInZeroInputReserve() public {
        vm.expectRevert(encodeError("InsufficientLiquidity()"));
        ReanswapV2Library.getAmountIn(1000, 0, 1.5 ether);
    }

    function testGetAmountInZeroOutputReserve() public {
        vm.expectRevert(encodeError("InsufficientLiquidity()"));
        ReanswapV2Library.getAmountIn(1000, 1 ether, 0);
    }
    ...
    function testGetAmountsIn() public {
        ...
        uint256[] memory amounts = ReanswapV2Library.getAmountsIn(
            address(factory),
            0.1 ether,
            path
        );
    ...
    function testGetAmountsInInvalidPath() public {
        ...
        vm.expectRevert(encodeError("InvalidPath()"));
        ReanswapV2Library.getAmountsIn(address(factory), 0.1 ether, path);
    }
}